...
import { startBookingEventsConsumer } from './messaging/booking-events.consumer';
...
class App {
    ...
    public start(): void {
        ...
        const rabbitReady = connectRabbitMQ();

        // Дожидаться обоих перед стартом consumer'а обязательно: если очередь
        // уже содержит сообщения (например, после простоя restaurants), канал
        // может прислать их раньше, чем dataSource.initialize() успеет
        // построить метаданные сущностей — тогда getRepository(NotificationLog)
        // упадёт с EntityMetadataNotFoundError.
        Promise.all([dbReady, rabbitReady]).then(([, channel]) => {
            if (channel && dataSource.isInitialized) {
                startBookingEventsConsumer(channel);
            }
        });
